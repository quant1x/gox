...
func parseObject(s string, c *cache) (*Value, string, error) {
	...
	for {
		...
		s = skipWS(s)
		if len(s) == 0 {
			return nil, s, fmt.Errorf(`cannot find object key`)
		}
		unquotedKey := false
		pos_start := 1
		if s[0] != '"' && s[0] != '\'' {
		...
		kv.k, s, err = parseRawKey(s[pos_start:], unquotedKey)
		if err != nil {
...
func parseRawKey(s string, unquotedKey bool) (string, string, error) {
	for i := 0; i < len(s); i++ {
		if unquotedKey && s[i] == ':' {
			return s[:i], s[i:], nil
		}
...
func (v *Value) GetInt64(keys ...string) int64 {
	...
	return ParseInt64BestEffort(v.s)
}
...
func (v *Value) GetUint64(keys ...string) uint64 {
	...
	return ParseUint64BestEffort(v.s)
}
