...
func GetSet[K comparable, V any](m ConcurrentHashMap[K, V], finished chan struct{}) (set func(key K, value V), get func(key K, value V)) {
...
func runWithShards(bench func(b *testing.B), b *testing.B, shardsCount int) {
	oldShardsCount := __SHARD_COUNT
	__SHARD_COUNT = shardsCount
	bench(b)
	__SHARD_COUNT = oldShardsCount
}
