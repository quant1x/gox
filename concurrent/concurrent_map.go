...
// ConcurrentHashMap A "thread" safe map of type string:Anything.
...
func create[K comparable, V any](sharding func(key K) uint32) ConcurrentHashMap[K, V] {
	m := ConcurrentHashMap[K, V]{
		sharding: sharding,
		shards:   make([]*ConcurrentMapShared[K, V], __SHARD_COUNT),
	}
...
func (m *ConcurrentHashMap[K, V]) GetShard(key K) *ConcurrentMapShared[K, V] {
	return m.shards[uint(m.sharding(key))%uint(__SHARD_COUNT)]
}
