...
func checkTopKInvariants[E any](x []E, k int, less func(E, E) int) bool {
	...
	if less(x[k-1], sorted[k-1]) < 0 || less(sorted[k-1], x[k-1]) < 0 { // x[k-1] != sorted[k-1]
		return false
	}
...
func TestTopKFunc(t *testing.T) {
	...
	less := func(x, y person) int { return x.age - y.age }
	for _, c := range cases {
		x := slices.Clone(c.x)
		TopKFunc(x, c.k, less)
		if !checkTopKInvariants(x, c.k, less) {
